                ...
                <div class="col-auto btn-group">
                    <select class="custom-select btn" id="inlineFormCustomSelect">
                        ...
                        @foreach($kongjian as $xl)
                            <option value="{!! $xl->id !!}" @if(request( 'cid',0)==$xl->id) selected @endif data-url="{{route('favorite',['cid'=>$xl->id,'vid'=>request('vid',0),'xid'=>request('xid',0)])}}">{{$xl->name}}</option>
                        @endforeach
                    </select>
                    <select class="custom-select btn" id="inlineFormCustomSelect">
                        <option selected>选择风格</option>
                        @foreach($fengge as $xl)
                            <option value="{!! $xl->id !!}" @if(request( 'xid',0)==$xl->id) selected @endif data-url="{{route('favorite',['xid'=>$xl->id,'vid'=>request('vid',0),'cid'=>request('cid',0)])}}">{{$xl->name}}</option>
                        @endforeach
            ...
            <div class="chose-option row float-left">
                <div class="col-auto btn-group">
                    <select class="custom-select" id="inlineFormCustomSelect">
                        ...
                        @foreach($xilie as $xl)
                            <option data-url="{{route('favorite',['vid'=>$xl->id,'xid'=>request('xid',0),'cid'=>request('cid',0)])}}" value="{!! $xl->id !!}" @if(request( 'xid',0)==$xl->id) selected @endif>{{$xl->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
